...
function vpb_multiple_file_uploader(vpb_configuration_settings)
{
    ...
    vpb_multiple_file_uploader.prototype.vpb_show_added_files = function(vpb_value)
	{
            ...
            this.vpb_files = vpb_value;
                totalfiles = this.vpb_files.length;
                finaltotalfiles =totalfiles;
                finaltotalfiles++;
               // console.log(this.vpb_files.length);
            console.log('Total file = '+totalfiles+'Final Total = '+finaltotalfiles);
...
//				vpb_file_id = files_name_without_extensions.replace(/[^a-z0-9\s]/gi, '').replace(/[_\s]/g, '');
				...
				if(typeof this.vpb_files[i] != undefined && this.vpb_files[i].name != "")
				{
					...
					vpb_added_files_displayer += '<tr id="add_fileID'+vpb_file_id+'" class="'+new_classc+'"><td>'+vpb_file_icon+' '+this.vpb_files[i].name.substring(0, 40)+'</td><td><div id="uploading_'+vpb_file_id+'" class="progress"><div class="progress-bar"></div></div></td><td>'+vpb_actual_fileSize+'</td><td><span id="remove'+vpb_file_id+'"><span class="vpb_files_remove_left_inner text-danger" onclick="vpb_remove_this_file(\''+vpb_file_id+'\',\''+this.vpb_files[i].name+'\');">Remove</span></span></td></tr></div>';
//<span id="uploading_'+vpb_file_id+'"><span style=color:blue;>Ready</span></span>\n\
	...
	vpb_multiple_file_uploader.prototype.vasPLUS = function(file,file_counter)
	{
		if(typeof file[file_counter] != undefined && file[file_counter] != '')
		{
			...
			//var ids = files_name_without_extensions;
			var vpb_browsed_file_ids = $("#"+this.vpb_settings.vpb_form_id).find("input[type='file']").eq(0).attr("id");
         ...
         var clientName = document.getElementById('ClientN').value;
...
//        var caseNAme=JSON.stringify(caseName);
       // var clientNAme = JSON.stringify(clientName);
				...
				dataString.append('upload_file_ids',ids);
                                dataString.append('caseName',caseName);
                                dataString.append('clientName',clientName);
                                  dataString.append('totalfiles',totalfiles);
                              //  dataString.append('Srno',Srno);
...
//                        var percentComplete = Math.round(percentComplete);
...
//                   $(".progress-bar").hide();
//                    $("#uploading_"+ids).html('<div align="center" class="mt-2" style="font-family:Verdana, Geneva, sans-serif;font-size:11px; text-align=center; margin-top:2px;color:Geen;">Completed</div>');
//                   // $('#uploadForm')[0].reset();
//                   $("#remove"+ids).html('<div align="center" style="font-family:Verdana, Geneva, sans-serif;font-size:11px;color:Green;">Uploaded</div>');
//                    
//                    var url = "http://apajuris.in/work/ClientBrief.php";
//                          //console.log(url);
//                       // window.location.href = url;
//                  //  $("#remove"+vpb_file_id).html('<p style="color:#28A74B;">uploaded</p>');
...
////                                            $("#uploading_"+ids).val(percentComplete+'%');
////                                            console.log(">--> "+percentComplete);
////                                        },
					success:function(response) 
					{
						...
						setTimeout(function() {
							var response_brought = response.indexOf(ids);
                                                       // var response_brought = response.indexOf(totalfiles);
							if ( response_brought != -1) {
                                                             $(".progress-bar").hide();
                                                               $("#uploading_"+ids).html('<div align="center" class="mt-2" style="font-family:Verdana, Geneva, sans-serif;font-size:11px; text-align=center; margin-top:2px;color:Geen;">Completed</div>');
...
//                    
								...
								//$("#remove"+ids).html('<div align="center" style="font-family:Verdana, Geneva, sans-serif;font-size:11px;color:gray;">Uploaded</div>');
                                                               // var url = "http://apajuris.in/work/CreateClientBreif.php?v="+file[file_counter].name;
                                                                 var url = "http://apajuris.in/work/ClientBrief.php";
                                                                //console.log(url);
                                                                
                                                                console.log(finaltotalfiles);
                                                                var response_brought = response.includes('_!r'+finaltotalfiles);
                                                                console.log(">====> "+response_brought);
							if ( response_brought == true) {
                                                             console.log("... inside..."+finaltotalfiles);
                                                                 console.log(url);
                                                            window.location.href = url;
                                                        }
